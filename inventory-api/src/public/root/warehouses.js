...
async function loadWarehouses() {
  const response = await fetch('/api/warehouses/company', {
    headers: { Authorization: `Bearer ${session.token}` },
  });
  const data = await response.json();

  if (!response.ok) {
    throw new Error(data.message || 'No se pudieron cargar las bodegas');
  }
...
warehouseForm.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(result.message || 'No se pudo crear la bodega');
    }
