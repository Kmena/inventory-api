...
if (!session?.token || !session?.user?.companyId || !['admin', 'sales_supervisor'].includes(session?.user?.role?.code)) {
  window.location.href = '/';
}

const authHeaders = () => routesShared.authHeaders(session);
...
async function loadOverview() {
  const response = await fetch('/api/sales-routes/company', {
    headers: { Authorization: `Bearer ${session.token}` },
  });
  const data = await response.json();
  if (!response.ok) {
    throw new Error(data.message || 'No se pudo cargar la consola de rutas');
  }
...
async function loadRouteDetail() {
  ...
  const response = await fetch(`/api/sales-routes/company/${selectedRouteId}`, {
    headers: { Authorization: `Bearer ${session.token}` },
  });
  const data = await response.json();
  if (!response.ok) {
    throw new Error(data.message || 'No se pudo cargar el detalle de la ruta');
  }
...
async function saveRoute() {
  ...
  const data = await response.json();
  if (!response.ok) {
    throw new Error(data.message || 'No se pudo guardar la ruta');
  }
...
async function saveSubzones() {
  ...
  const response = await fetch(`/api/sales-routes/company/${selectedRouteId}/subzones`, {
    method: 'PUT',
    headers: authHeaders(),
  ...
  const data = await response.json();
  if (!response.ok) {
    throw new Error(data.message || 'No se pudieron guardar las subzonas');
  }
...
async function saveAssignments() {
  ...
  const response = await fetch(`/api/sales-routes/company/${selectedRouteId}/assignments`, {
    method: 'PUT',
    headers: authHeaders(),
  ...
  const data = await response.json();
  if (!response.ok) {
    throw new Error(data.message || 'No se pudieron guardar los agentes');
  }
...
async function saveGoals() {
  ...
  const response = await fetch(`/api/sales-routes/company/agents/${selectedGoalsAgentId}/goals`, {
    method: 'PUT',
    headers: authHeaders(),
  ...
  const data = await response.json();
  if (!response.ok) {
    throw new Error(data.message || 'No se pudieron guardar las metas');
  }
