...
if (!session?.token || !session?.user?.companyId) {
...
async function loadDashboard() {
  const response = await fetch('/api/agent/dashboard', { headers: authHeaders() });
...
async function loadStores() {
  ...
  const response = await fetch(`/api/agent/stores?${params.toString()}`, { headers: authHeaders() });
  const data = await response.json();
  if (!response.ok) throw new Error(data.message || 'No se pudo cargar el recorrido');
...
async function loadGoals() {
  const response = await fetch('/api/agent/goals', { headers: authHeaders() });
  const data = await response.json();
  if (!response.ok) throw new Error(data.message || 'No se pudieron cargar las metas');
...
async function loadSelectedStoreDetail() {
  ...
  const response = await fetch(`/api/agent/stores/${selectedStoreId}`, { headers: authHeaders() });
  const data = await response.json();
  if (!response.ok) throw new Error(data.message || 'No se pudo cargar la ficha de la tienda');
  selectedStoreDetail = data;
  renderStoreDetail();
}
