...
async function loadContext() {
  const response = await fetch(`/api/agent/stores/${storeId}/order-context`, { headers: authHeaders() });
  const data = await response.json();
  if (!response.ok) throw new Error(data.message || 'No se pudo cargar el contexto del pedido');
  orderContext = data;
  render();
}
...
form.addEventListener('submit', async (event) => {
  ...
  try {
    const response = await fetch(`/api/agent/stores/${storeId}/orders`, {
      method: 'POST',
      headers: authHeaders(),
      body: JSON.stringify(payload),
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.message || 'No se pudo crear el pedido');
    form.reset();
    render();
