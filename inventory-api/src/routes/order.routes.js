...
router.post('/', authorizePermission('sales.manage', 'sales.orders.create'), validate(createOrderSchema), async (req, res, next) => {
  try { return res.status(201).json(await orderService.createOrder(req.body, req.auth)); } catch (error) { return next(error); }
});

router.put('/:id', authorizePermission('sales.manage', 'sales.orders.create'), validate(updateOrderSchema), async (req, res, next) => {
  try { return res.json(await orderService.updateOrder(parseBigIntId(req.params.id), req.body, req.auth)); } catch (error) { return next(error); }
});

router.post('/:id/approve', authorize('admin', 'sales'), async (req, res, next) => {
  try { return res.json(await orderService.approveOrder(parseBigIntId(req.params.id), req.auth)); } catch (error) { return next(error); }
});

router.post('/:id/cancel', authorize('admin', 'sales'), async (req, res, next) => {
  try { return res.json(await orderService.cancelOrder(parseBigIntId(req.params.id), req.auth)); } catch (error) { return next(error); }
});

router.post('/:id/dispatch', authorize('admin', 'warehouse'), async (req, res, next) => {
  try { return res.json(await orderService.dispatchOrder(parseBigIntId(req.params.id), req.auth)); } catch (error) { return next(error); }
});

router.delete('/:id', authorize('admin'), async (req, res, next) => {
  try {
    await orderService.removeOrder(parseBigIntId(req.params.id), req.auth);
