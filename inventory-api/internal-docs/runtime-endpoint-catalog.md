...
1. `covered-by-openapi`: la operación aparece en `internal-docs/openapi/runtime-baseline.openapi.json` con `x-runtime-source`.
2. `intentionally-excluded`: la operación aparece en `internal-docs/runtime-contract-manifest.json` con `reasonCode` y razón explícita.
...
Las exclusiones actuales viven en `internal-docs/runtime-contract-manifest.json`. Option B satisfied para la superficie mínima crítica documentada en `internal-docs/critical-contract-matrix.json`. Las categorías activas son:
...
| POST | `/api/auth/logout` | Sí | `authenticate` | Cerrar la sesión autenticada actual | Invalida browser session cuando aplica y limpia cookies |
