...
async function loadEconomicActivities(query = '') {
  try {
    const activities = await inventoryAuth.fetchJson(session, `/api/economic-activities?q=${encodeURIComponent(query)}`);
    renderEconomicActivities(activities);
  } catch (_error) {
    // Mantener el formulario util aunque el catalogo falle temporalmente.
  }
}
...
async function searchMapPlaces(query) {
  try {
    return await inventoryAuth.fetchJson(session, `/api/geocoding/search?q=${encodeURIComponent(query)}`, {
      fallbackMessage: 'No se pudo buscar la ubicacion',
    });
  } catch (_error) {
...
async function loadZones() {
  const data = await inventoryAuth.fetchJson(session, '/api/regions/company', {
...
async function loadClassifications() {
  const data = await inventoryAuth.fetchJson(session, '/api/clients/classifications/company', {
...
async function loadDocumentTypes() {
  const data = await inventoryAuth.fetchJson(session, '/api/clients/document-types', {
...
async function loadClients() {
  const data = await inventoryAuth.fetchJson(session, '/api/clients/company', {
...
lookupTaxpayerButton.addEventListener('click', async () => {
  ...
  try {
    ...
    const result = await inventoryAuth.fetchJson(session, `/api/taxpayers/lookup?${params.toString()}`, {
      fallbackMessage: 'No se encontraron datos en Hacienda',
    });
...
clientForm.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    const result = await inventoryAuth.fetchJson(session, url, {
      method: editingClientId ? 'PUT' : 'POST',
      body: JSON.stringify(payload),
      fallbackMessage: editingClientId ? 'No se pudo actualizar el cliente' : 'No se pudo crear el cliente',
    });

    const targetClientId = editingClientId || result.id;
    for (const store of pendingStores) {
      await inventoryAuth.fetchJson(session, `/api/clients/company/${targetClientId}/stores`, {
        method: 'POST',
        body: JSON.stringify(store),
        fallbackMessage: `No se pudo crear la tienda ${store.name}`,
      });
    }

    for (const reference of pendingClientReferences) {
      await inventoryAuth.fetchJson(session, `/api/clients/${targetClientId}/references`, {
        method: 'POST',
        body: JSON.stringify(reference),
        fallbackMessage: `No se pudo guardar la referencia ${reference.name}`,
      });
    }

    for (const document of pendingClientDocuments) {
      await inventoryAuth.fetchJson(session, `/api/clients/${targetClientId}/documents`, {
        method: 'POST',
        body: JSON.stringify(document),
        fallbackMessage: `No se pudo subir el documento ${document.fileName}`,
      });
    }
