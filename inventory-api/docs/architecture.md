...
- a dedicated recipes adapter plus helper/state/renderer seams through `root/recipes-api.js`, `root/views/recipes-admin.helpers.js`, `root/views/recipes-admin.state.js`, and `root/views/recipes-admin.renderers.js`, plus the dedicated registered version-editor seam `root/views/recipes-admin.version-editor.js`, with `#recetas` currently implemented as a usable administrative workflow for list/detail/filter/create/version/approve/assignment behavior over the existing recipe and product contracts; static workspace markup now delegates through `renderWorkspace()` in the renderer module, while dynamic stage and QA version-editor behavior is isolated behind `views.recipesAdminVersionEditor`; the current modularity baseline keeps `recipes-admin.js` at 569 lines and no affected file above 600 lines; the active model remains explicit that products are assigned to recipes through `product.recipeId`, while order execution freezes a concrete recipe version through `productionOrder.recipeVersionId` and `recipeVersionSnapshot`; there is still no separately proven persisted product-specific applicable recipe-version binding;
...
- **Warehouse/QA shell (`src/public/warehouse/**`)**: supported authenticated warehouse entrypoint, session bootstrap through shared same-origin helpers, bounded `window.WarehouseShell` registry seam (`register`, `require`, `has`), permission-gated hash router with `TAB_DEFINITIONS`-based permission check (hidden entries gate sub-views without exposing them as tabs), bottom-tab/rail navigation, receipts workflow UI (4-step: arrival → inspection → evidence → confirm), and a now-explicit modular production browser slice where `views/production.js` stays a thin orchestrator delegating to `views/production.state.js` (pure stage-status derivation, including `WAITING_QA` gating from `qaOutOfTolerance` plus inspection approval semantics), `views/production.renderers.js` (order/stage/lot-picker/QA HTML generation), and `views/production.controllers.js` (inline form wiring, lot assignment, QA capture, override handling, and payload collection); the active execute-stage UI now renders FEFO/FIFO lot assignment and inline numeric QA capture for `qaMandatory` snapshot stages, reuses a shared `overrideJustification` client flow for QA out-of-tolerance and over-consumption warnings, and keeps the complete-order section guarded by `canCompleteProduction`; a dedicated `receive-from-po.js` sub-view implements a two-phase receive-from-purchase-order workflow (phase 1: select ISSUED PO from `GET /api/receipts/purchase-orders`; phase 2: fill actual quantities per item and select destination warehouse; submit to `POST /api/receipts`), alongside read-only frozen recipe consultation, progressive-enhancement capture helpers for barcode/camera/manual flows, warehouse identity/logout controls, explicit local-only photo-evidence messaging for the current cycle, and bounded runtime-governance coverage enforced by `tests/warehouse-spa-runtime.test.js`, `tests/warehouse-receive-production-task001.test.js`, `tests/warehouse-receive-production-task002-007.test.js`, `tests/public-runtime-http-smoke.test.js`, and `scripts/validate-public-runtime.js`; `state.js` derives `canReceive` from `receipts.inspect`, `canExecuteProduction` from `production.execute`, `canCompleteProduction` from `production.complete`, and `canViewProduction` from `production.view`; `warehouse-api.js` now exposes `listPurchaseOrdersForReceipt`, `createReceipt`, `listWarehouses`, `startProductionOrder`, `completeProductionOrder`, `getMaterialRequirements`, and `getAvailableLotsForStage`, and the runtime characterization suite now explicitly freezes those two production-planning wrapper paths and registrations plus the current production-module split
...
- the warehouse shell is governed by `validate-public-runtime`, the same bounded `typecheck` baseline, `tests/warehouse-spa-runtime.test.js`, `tests/public-surface-characterization.test.js`, `tests/public-runtime-http-smoke.test.js`, `tests/post-audit-baseline-hardening.test.js`, and the broader browser E2E source baseline that now expects `/warehouse/` as the supported landing path; the warehouse runtime characterization suite now explicitly freezes the `getMaterialRequirements(...)` and `getAvailableLotsForStage(...)` wrapper registrations and approved route paths inside `warehouse-api.js`.
- `root/index.html` and `scripts/validate-public-runtime.js` now actively govern the approved inventory, supplier, quotations, dedicated RFQ tracking browser modules, the recipes version-editor seam, plus the warehouses/products/movements/quotations view helper-renderer-state-controller scripts and the lots view modules as part of the supported shell loader contract; adding, removing, or reordering those scripts is a contract change. `src/public/root/runtime-contract.js` now includes the RFQ tracking script inventory together with `views.recipesAdminVersionEditor`, so loader-governance truth is synchronized across `index.html`, tests, validator expectations, and the runtime-contract source.
...
- the latest user-supplied validation baseline for the current repository state reports `node --test tests/warehouse-spa-runtime.test.js` ✅, targeted eslint for the changed warehouse production files plus `tests/warehouse-spa-runtime.test.js` ✅, and `npm run build` ✅ after the warehouse production module split and inline QA/override flow hardening; the same current truth now also includes `recipes-production-qa-execution-hardening` `TASK-010` evidence for `node --test tests/root-shell-recipes-admin-view-characterization.test.js tests/root-shell-modularity-governance.test.js` ✅, targeted eslint ✅, and `npm run build` ✅ after the root recipes-admin seam split; `npm run typecheck` is currently known-red only because of pre-existing unrelated landing typing failures outside the recipes/production hardening slice, and full `npm test` is currently known-red because of pre-existing governance-baseline/documentation drift failures outside this warehouse slice, so the older repository-wide green typecheck/full-suite baselines are historical evidence rather than current truth
- manual browser validation is still an open verification step for the current warehouse production execute-stage UX; the active architecture is characterized by the runtime tests and supplied build/lint evidence, not by completed end-to-end browser evidence for this exact slice
...
- whether the current `#recetas` workflow should keep the now-implemented renderer + registered version-editor seam split or receive a further boundary split beyond `recipes-admin.js` if future approved slices add more forms, pagination, or richer product-version traceability behavior
