...
- `recipe.service.js` currently enforces company-scoped recipe access by `req.auth.companyId`, validates nested ingredient/stage-input `productId` references against the authenticated company before persistence, now also rejects product-linked stage inputs whose `unit` does not match the referenced company product `unit`, allocates sequential `versionNumber` values per recipe, and treats approved versions as immutable;
- `recipe.schema.js` now formalizes numeric expected QA parameters for `qaMandatory` stages, requires `stageInput.unit` when `productId` is present, and rejects `qaMandatory=true` stages that omit `expectedParameters`;
...
- `production.service.js` currently validates product, recipe-version, warehouse, and responsible-user references against `req.auth.companyId`, enforces sourcing/approved-recipe guardrails with `production.override` justification when needed, stores a JSON-safe frozen `recipeVersionSnapshot` so later recipe master-data changes do not rewrite existing production orders, wraps production-order create/approve planning inside Prisma inventory transactions guarded by `acquireCompanyInventoryAdvisoryLock(...)`, and now remains a 591-line façade that keeps order creation/approval/start/cancel orchestration, serialization, shared planning helpers, and the stable public exports `executeProductionStage`, `recordProductionReturn`, `reconcileProductionOrderAggregates`, and `completeProductionOrder`; `src/services/production-planning.service.js` currently owns `buildRecipeVersionSnapshot`, `buildMaterialRequirements`, `assertStockAvailability`, and `buildEnrichedSnapshot`, and the active create/approve flow now uses those helpers to compute material requirements, persist `ProductionOrderMaterialRequirement` rows, enrich `recipeVersionSnapshot.materialRequirements`, revalidate stock availability on approval, and record `PRODUCTION_ORDER_OVERRIDE_STOCK` audit events when request context is available. `buildRecipeVersionSnapshot(...)` now also explicitly normalizes each stage `expectedParameters` entry to the formal QA contract `{ name, unit, expectedValue, minTolerance, maxTolerance }`, omits empty legacy `parameterTolerances`, and preserves non-empty legacy `parameterTolerances` arrays for backward compatibility; that normalized snapshot shape is reused by `buildEnrichedSnapshot(...)` and `buildOrderSnapshotWithMaterialRequirements(...)` for create/approve payloads without changing routes or the database schema. `src/services/production-material-availability.service.js` now exists as a dedicated read-model seam for `GET /api/production/orders/:id/material-requirements` and `GET /api/production/orders/:id/stages/:stageId/available-lots`: both reads are tenant-scoped through the authenticated `companyId`, the material-requirements read combines persisted `production_order_material_requirements` with current origin-warehouse stock to return `required/available/missing`, and the available-lots read resolves stage inputs from the frozen snapshot while looking up `requiresLot` / `requiresExpiration` from the current tenant product catalog because those flags are not frozen in the current snapshot shape; it filters to sellable lots, orders FEFO when expiration applies and FIFO by `entryDate` otherwise, and omits `internalLotNumber` from the response. On the browser side, `src/public/warehouse/api/warehouse-api.js` already exposes same-origin wrappers for both reads through `getMaterialRequirements(...)` and `getAvailableLotsForStage(...)`, and `tests/warehouse-spa-runtime.test.js` now explicitly characterizes those wrapper registrations and route paths as part of the current supported warehouse runtime contract. `src/services/production-execution.service.js` now owns the current execution/return/completion aggregate logic extracted ahead of the originally planned phase split by explicit user authorization so `TASK-005` could satisfy the approved size ceiling without changing the public API surface, while the delegated `production.service.js` façade preserves `__private__` compatibility and continues exporting the same execution-facing entry points; the extracted execution service now resolves stage execution targets from the frozen snapshot, enforces stage-prerequisite sequencing before execution, validates consumption totals against persisted per-order material requirements using the currently approved temporary `0.05` tolerance fallback, requires `production.override` plus explicit justification when consumption or QA tolerance violations occur, persists enriched QA parameter rows together with `qaOutOfTolerance` and `overrideJustification`, records request-aware override audit events through the new focused helper seam `src/services/production-stage-validation.service.js`, reduces stock transactionally through the existing inventory helper flow using `movementType: OUT` with `reasonCode: PRODUCTION_CONSUMPTION` and `PRODUCTION_WASTE`, records explicit lot-bound raw-material devoluciones as `movementType: IN` linked to `ProductionReturn` records, blocks order completion unless all mandatory QA gates pass, creates the finished-goods lot during completion, posts a destination-warehouse `movementType: IN` receipt with active `reasonCode: PRODUCTION_RECEIPT`, transitions the order to `COMPLETED`, synchronizes `ProductionItem.consumedQuantity` after consumption writes through `syncProductionItemConsumedQuantity`, and exposes `reconcileProductionOrderAggregates` as the current service-level repair path for downstream aggregate drift.
- the current stage-execution payload validation requires `startedAt`, `endedAt`, optional actual parameters, evidence, consumption rows, waste rows, and rejects `endedAt < startedAt`; persisted execution records now capture stage order/name, responsible user, actual parameters, evidence, notes, a `movementGroupId` used to correlate related stock deductions, and additive QA persistence fields `qaOutOfTolerance` plus nullable `overrideJustification`; the runtime now populates those fields from request handling, stores enriched expected-vs-actual QA measurements, and returns override-related conflict/validation subcodes when sequencing, consumption, or tolerance rules are violated without authorized justification;
...
- the validation/documentation closure for the current supply slices is also now reflected: targeted tests, focused eslint, `npm run typecheck`, `npm run build`, and `npm test -- --silent` were reported green for the user-authorized structural split that closed `recipes-production-qa-execution-hardening` `TASK-005` acceptance around `src/services/production.service.js <= 600` while keeping API compatibility through delegation to `src/services/production-execution.service.js`; additional user-supplied validation evidence for `recipes-production-qa-execution-hardening` `TASK-006` reports targeted tests ✅, focused eslint ✅, `npm run typecheck` ✅, and `npm test -- --silent` ✅ for the snapshot QA-contract normalization covered by `tests/production-planning.service.test.js` and `tests/production-service-foundation.test.js`; `node --test tests/production-service-foundation.test.js tests/production-schema.test.js tests/production-consumption-hardening-migration.test.js`, `npm run lint`, `npm run typecheck`, `npm run build`, and `npx prisma validate --schema prisma/schema.prisma` passed for `TASK-006`; `node --test tests/production-service-foundation.test.js tests/production-schema.test.js tests/production-waste-hardening-migration.test.js`, `npm run lint`, `npm run typecheck`, `npm run build`, and `npx prisma validate --schema prisma/schema.prisma` passed for `TASK-007`; `node --test tests/production-service-foundation.test.js tests/production-schema.test.js tests/production-routes-contract.test.js tests/runtime-contract-governance.test.js tests/production-return-foundation-migration.test.js`, `npm run lint`, `npm run typecheck`, `npm run build`, and `npx prisma validate --schema prisma/schema.prisma` passed for `TASK-008`; final user-supplied validation evidence for `recipes-production-qa-execution-hardening` Phase 4 / `TASK-008` now additionally reports `node --test tests/production-execution.service.test.js tests/quality-inspection-foundation.test.js tests/production-schema.test.js` ✅, `npm run typecheck && npm run lint && npm run build && npm test -- --silent` ✅, and a full-suite result of `1232` passing, `0` failing, `2` skipped after the final helper extraction kept `src/services/production.service.js` at 591 lines and `src/services/production-execution.service.js` at 582 lines; user-supplied validation evidence for the current `TASK-009` reconciliation cycle reports `node --test tests/warehouse-spa-runtime.test.js` ✅, `npx eslint tests/warehouse-spa-runtime.test.js` ✅, and `npm test -- --silent` ✅ after adding explicit warehouse wrapper characterization coverage for `getMaterialRequirements(...)` and `getAvailableLotsForStage(...)`; `npm run typecheck` currently remains red only because of pre-existing unrelated permission-metadata typing failures outside this feature scope; user-supplied validation evidence for `recipes-production-qa-execution-hardening` `TASK-010` reports `node --test tests/root-shell-recipes-admin-view-characterization.test.js tests/root-shell-modularity-governance.test.js` ✅, targeted eslint ✅, `npm run build` ✅, and `npm test -- --silent` ✅ after the root recipes admin view delegated static workspace markup to `renderWorkspace()` and moved dynamic stage/QA version-editor behavior into the dedicated `views.recipesAdminVersionEditor` seam; `npm run typecheck` remains red only because of pre-existing unrelated permission-metadata typing failures outside this feature scope; user-supplied validation evidence for `TASK-011` reports 59 tests passed after adding 12 completion-focused tests, together with `npm run lint`, `npm run typecheck`, and `npm run build`; user-supplied validation evidence for `TASK-012` reports 79 tests passed together with `npm run lint`, `npm run typecheck`, and `npm run build`; user-supplied validation evidence for `TASK-013` reports 87 tests passed together with `npm run lint`, `npm run typecheck`, and `npm run build`; user-supplied validation evidence for `TASK-014` plus `TASK-015` reports 102 tests passed with all validations green, together with `npm run lint`, `npm run typecheck`, and `npm run build`; user-supplied validation evidence for `recipes-production-qa-execution-hardening` Fase 1 now reports `npm run prisma:deploy`, `npm run build`, `npm run typecheck`, `node --test tests/recipe-schema.test.js`, `node --test tests/recipe-service-foundation.test.js`, and `npm test -- --silent` green (`1205` pass, `0` fail, `2` skipped); and focused backend coverage now also includes `tests/production-planning.service.test.js`, `tests/inventory-transaction-support.service.test.js`, and the updated `tests/production-service-foundation.test.js` regression path for centralized snapshot building. In the documented warning posture, the previously noted recipes/production permission-catalog gap for already-provisioned databases is now closed by additive migration; remaining non-warehouse risks stay concentrated in partially app-enforced production invariants, missing DB-enforced `warehouseId` / `productId` integrity on production detail rows, no standalone `/api/quality` route group, external billing handoff still pending, the unresolved stage-execution active-state/idempotency contradiction (`DEC-016` assumes nullable active `endedAt`, but the current schema/runtime still require `endedAt`), pre-existing unrelated Windows Prisma file-lock/full-suite characterization issues, and broader Playwright-lane instability outside the bounded runtime-governance lane.
...
- `root/views/recipes-admin.renderers.js`
- `root/views/recipes-admin.version-editor.js`
...
- a company-admin Recipes route at `#recetas` with dedicated RootShell registration, manifest wiring, runtime-contract registration, `recipesApi` browser adapter coverage for `/api/recipes/**`, helper/state/renderer seams, a dedicated registered version-editor seam in `views.recipesAdminVersionEditor`, and a usable administrative workflow that loads recipe lists plus detail/version data, applies local search/status/type/shared filters, allows root-side recipe creation, draft version create/edit, version approval, and recipe-to-product assignment through the existing product update contract while keeping explicit UI language when a product-specific applied recipe version is not visible in the current model; static workspace markup now delegates through `renderWorkspace()` in `recipes-admin.renderers.js`, while dynamic stage and QA version-editor orchestration lives in `recipes-admin.version-editor.js`; the current hardening baseline also keeps `recipes-admin.js` at 569 lines and no affected file above 600 lines; current code proves `product.recipeId` assignment plus order-level `productionOrder.recipeVersionId` / `recipeVersionSnapshot`, but does not prove a persisted product-specific `product -> recipe -> applicable recipeVersion` binding independent from each order;
...
- let a tenant-scoped warehouse/QA actor with `warehouse.access` use the supported `/warehouse/` SPA, where navigation is permission-gated, receipts use a four-step operational workflow (arrival, inspection, evidence, confirm), and the production detail runtime is now explicitly split into `src/public/warehouse/views/production.js` (thin orchestrator), `production.state.js` (pure status helpers), `production.renderers.js` (HTML generation), and `production.controllers.js` (event wiring + payload collection); the execute-stage flow now renders inline FEFO/FIFO lot assignment plus inline numeric QA capture for `qaMandatory` snapshot stages, reuses a single `overrideJustification` client flow for both QA out-of-tolerance and lot over-consumption warning paths, derives `WAITING_QA` from the same `qaOutOfTolerance`/inspection-cleared semantics used by the backend completion gate, and keeps the complete-order section guarded by `canCompleteProduction`; a dedicated `receive-from-po.js` view implements a two-phase "Recibir desde OC" workflow where the operator first selects a pending `ISSUED` purchase order loaded from `GET /api/receipts/purchase-orders`, then fills actual-arrival quantities per item and selects a destination warehouse before submitting to `POST /api/receipts`; the receipts list now shows a "Nueva desde OC" button that navigates to this view when `canReceive` is true; `canReceive` is now derived from `receipts.inspect` (not the older `warehouse.receive`), and `canExecuteProduction`, `canCompleteProduction`, `canViewProduction` are now driven by the `production.*` permission flags; the `production_operator` role bundle now includes `warehouse.access` in its base permissions and `receipts.confirm` as a sensitive addition, while `warehouse_operator` now includes `production.view`/`production.execute` as allowed additions and `production.complete` as a sensitive addition; camera/barcode/manual fallback capture helpers remain same-origin browser-only behavior;
...
- `recipes-production-qa-execution-hardening` Fase 2 / `TASK-005` functional scope added migration `20260915001000_production_order_material_requirements` and Prisma model `ProductionOrderMaterialRequirement`; `production_order_material_requirements` is now the additive persistence boundary for per-order planned material needs linked to `production_orders` and company products.
...
- `/api/receipts` is now mounted and currently exposes `GET /`, `POST /`, `GET /:id`, `GET /purchase-orders` (new — returns `ISSUED` purchase orders for the authenticated company so the warehouse SPA receive-from-PO workflow can populate the PO selector), `POST /:id/items/:itemId/inspections`, `POST /:id/confirm`, `POST /:id/reverse`, `GET /:id/fiscal-references`, and `POST /:id/fiscal-references`; the `GET /purchase-orders` route is mounted before `GET /:id` to avoid Express route shadowing; these routes are authenticated, policy-guarded through `receipt.view`, `receipt.inspect`, `receipt.confirm`, and `receipt.reverse`, and now implement actual-arrival document capture, inspection-driven state transitions, transactional confirmation/reversal stock workflows, and pending fiscal-reference persistence without any external billing call;
...
- `production.override` is now consumed by `src/services/production.service.js` and `src/services/production-stage-validation.service.js`: production order guardrail overrides require explicit justification, production stage execution now also requires explicit justification for approved consumption/QA deviations, stage executions persist `overrideJustification`, production orders continue persisting their override justification, and request-aware audit events are recorded for stock-override and stage-execution override paths when request context is available. `procurement.override`, `quality.override`, and `inventory.intake.override` remain metadata-governed only through `requiresJustification: true` declarations and helper functions; receipt confirmation/reversal routes are now active, but they still do not persist a richer override-justification workflow artifact.
...
- the latest user-supplied validation baseline for the current warehouse production browser slice reports `node --test tests/warehouse-spa-runtime.test.js` ✅, targeted eslint for the changed warehouse production files plus `tests/warehouse-spa-runtime.test.js` ✅, and `npm run build` ✅ after the `production.js` split plus inline QA/override flow hardening; `npm run typecheck` remains red because of pre-existing unrelated landing typing failures outside this feature, and full `npm test` remains red because of pre-existing governance-baseline/documentation drift failures outside this feature; the older repository-wide green typecheck/full-suite baselines remain historical evidence rather than current truth
- manual browser validation for the new warehouse execute-stage QA/override UX is still pending; no interactive browser execution evidence beyond the supplied runtime characterization/build results was provided in this refresh
...
- user-supplied validation evidence for `supply-inventory-entry` `TASK-005` reports `PASS` for `node --test tests/production-service-foundation.test.js tests/production-schema.test.js tests/production-routes-contract.test.js tests/runtime-contract-governance.test.js`, together with `npm run lint`, `npm run typecheck`, `npm run build`, and `npx prisma validate`; user-supplied validation evidence for `TASK-006` reports `PASS` for `node --test tests/production-service-foundation.test.js tests/production-schema.test.js tests/production-consumption-hardening-migration.test.js`, `npm run lint`, `npm run typecheck`, `npm run build`, and `npx prisma validate --schema prisma/schema.prisma`; user-supplied validation evidence for `recipes-production-qa-execution-hardening` Fase 2 / `TASK-005` functional scope reports targeted tests ✅, focused eslint ✅, `npm run prisma:deploy` ✅, `npm run prisma:generate` ✅, `npm run typecheck` ✅, `npm run build` ✅, and `npm test -- --silent` ✅ after adding migration `20260915001000_production_order_material_requirements`, Prisma model `ProductionOrderMaterialRequirement`, transaction/advisory-lock-backed create/approve planning, persisted material requirements, snapshot enrichment, approval-time stock revalidation, request-aware audit correlation, and stock-override audit events; user-supplied validation evidence for `recipes-production-qa-execution-hardening` Fase 2 / `TASK-007` reports targeted service + route tests ✅, focused eslint ✅, `npm run typecheck` ✅, full test suite ✅, and `npm run build` ✅ after adding `src/services/production-material-availability.service.js`, mounting the tenant-scoped material-requirements and available-lots reads, and updating the canonical runtime-contract artifacts; final user-supplied evidence for `recipes-production-qa-execution-hardening` Phase 4 / `TASK-008` reports `node --test tests/production-execution.service.test.js tests/quality-inspection-foundation.test.js tests/production-schema.test.js` ✅, `npm run typecheck && npm run lint && npm run build && npm test -- --silent` ✅, full-suite `1232` pass / `0` fail / `2` skipped, and line-count closure at `production.service.js = 591` plus `production-execution.service.js = 582`; user-supplied validation evidence for `TASK-009` reports 31 tests passed after adding aggregate-synchronization and reconciliation coverage in `tests/production-service-foundation.test.js`, together with `npm run lint`, `npm run typecheck`, and `npm run build`; user-supplied validation evidence for `TASK-010` reports 53 tests passed after adding quality-inspection coverage; user-supplied validation evidence for `TASK-011` reports 59 tests passed after adding 12 completion-focused tests in `tests/production-completion.test.js`; local Windows build logs may still include handled Prisma retry noise even when the final build succeeds.
...
- the warehouse SPA `receive-from-po.js` view must continue implementing the two-phase PO-selection → form-fill workflow using `GET /api/receipts/purchase-orders` for PO loading and `POST /api/receipts` for submission; the PO selector must filter to `ISSUED`-status orders; the "Nueva desde OC" button in `receipts.js` must remain guarded by `canReceive`; `canReceive` must remain derived from `receipts.inspect` permission, not from `warehouse.receive`; and `canExecuteProduction`, `canCompleteProduction`, and `canViewProduction` in `state.js` must remain derived from the respective `production.*` permission flags.
- the `receive-from-po` route must remain declared in `TAB_DEFINITIONS` with `hidden: true` and permission guard `p.includes('receipts.inspect')` so that the hash-router permission gate applies; the `renderTabBar` function must skip entries with `hidden: true` so the sub-view never appears as a standalone visible tab; removing this hidden entry would silently allow any `warehouse.access` user to load the receive-from-po view regardless of `receipts.inspect` status.
- the `GET /api/receipts/purchase-orders` route must remain declared **before** `GET /api/receipts/:id` in `receipt.routes.js` to prevent Express route-parameter shadowing.
...
- `production.override` is now enforced by both `src/services/production.service.js` and `src/services/production-stage-validation.service.js`; current production flows persist override justification on production orders and stage executions and can emit request-aware audit events for stock-override and stage-execution override paths, but the repository still lacks a broader cross-workflow override artifact model. The new procurement routes currently use `procurement.view`, `procurement.manage`, and `procurement.approve` but do not yet consume `procurement.override`; `quality.override` and `inventory.intake.override` also remain metadata-only.
