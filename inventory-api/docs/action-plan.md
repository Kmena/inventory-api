...
- the remaining downstream follow-up is now concentrated in six non-warehouse areas: missing DB-enforced `warehouse_id` / `product_id` integrity on production detail tables, justification persistence/audit gaps around override-capable workflows, absence of standalone downstream `/api/quality` route groups, lack of external fiscal-document handoff for purchase orders/receipts, missing root-admin pages for the new receipt/fiscal flows, and DB-level hardening still missing for several lifecycle invariants that are currently enforced only in service/validation code
...
15. extend the first override-capable workflow baseline from `production.service.js` into a fuller persistence/audit contract so justification metadata is not only validated in-memory but also durably recorded when later workflow services are implemented;
...
- expanding override-justification handling from the current `production.service.js` validation baseline into durable persistence/audit behavior instead of treating it as metadata-only
