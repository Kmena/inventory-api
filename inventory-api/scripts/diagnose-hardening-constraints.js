...
const DIAGNOSTIC_QUERIES = [
  ...
  },
  {
    ...
    query: `SELECT id FROM payments WHERE amount < 0 ORDER BY id LIMIT 20`,
  },
  {
    key: 'invoices_negative_amount',
    description: 'Facturas con amount negativo.',
    query: `SELECT id FROM invoices WHERE amount < 0 ORDER BY id LIMIT 20`,
  },
  {
...
async function runDiagnostic() {
  ...
  for (const diagnostic of DIAGNOSTIC_QUERIES) {
    const rows = await prisma.$queryRawUnsafe(diagnostic.query);
    results.push({
      key: diagnostic.key,
